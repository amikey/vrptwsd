 ...
 * 
 */
public class SimilarityRemoval implements RemovalInterface {

	private static int seed = 0;
	Random r;

	public SimilarityRemoval() {
		seed++;
		r = new Random(seed);
	}

	public static void setSeed(int seed) {
		SimilarityRemoval.seed = seed;
	}
	 ...
	 */
	public List<RemovedCustomer> removeRemainingCustomers(Solution solution,
			int amountOfRemovedCustomers,
			List<RemovedCustomer> removedCustomers,
			Customer firstRemovedCustomer) {

		double maxDistanceOfAll = Double.MIN_VALUE;
		for (CustomerInTour c : solution.getUnfixedCustomersInTours()) { // -OB-
																			// Iteriert
																			// jetzt
																			// nur
																			// über
		...
		if (solution.getTours().size() >= amountOfRemovedCustomers - 1) {
			...
			for (Tour tour : solution.getTours()) {
				...
				for (CustomerInTour customer : tour.getUnfixedCustomersInTour()) {// -OB-Berechnet
																					// die
																					// similarity
																					// jetzt
																					// nur
	...
	private double calculateSimilarity(Customer firstRemovedCustomer,
		...
		return calculateSimilarity(firstRemovedCustomer,
				customer.getCustomer(), maxDistanceOfAll);
	}

	public double calculateSimilarity(Customer firstRemovedCustomer,
			Customer customer, double maxDistanceOfAll) {

		double distance = new Edge(firstRemovedCustomer, customer).getLength();

		double firstDivisor = distance / maxDistanceOfAll;

		Customer i = firstRemovedCustomer;
		Customer j = customer;
		double tauIJ = Math
				.max(1,
						(Math.min(j.getTimeWindowClose(),
		...
		// System.out.println("sim Val of cust " +
		// firstRemovedCustomer.getCustomerNo() + " and cust " +
		// customer.getCustomerNo() + ": " + term);
