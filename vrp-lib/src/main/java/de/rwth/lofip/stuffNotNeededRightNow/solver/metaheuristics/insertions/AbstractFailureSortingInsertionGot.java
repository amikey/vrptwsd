 ...
 * 
 */
public abstract class AbstractFailureSortingInsertionGot implements
    ...
    public SolutionGot insertCustomers(SolutionGot solution,
        ...
        for (RemovedCustomer customerToInsert : customerList) {
            ...
            // first route where it is possible to retain the constraint (total
            // expected demand below 2Q)
            // within that tour, from all possible insertion points, choose
            // the cheapest one (lowest additional cost).
            ...
            if (chosenTour == null) 
            {
            	...
            	solution.createNewTourWithCustomer(customerToInsert.getCustomer(), iteration, getClass().getSimpleName());
            } else {
                // case 2: we do have a possible tour to insert in. Take
                // that and calculate the cheapest insertion point
            	GroupOfTours got = solution.findGotForTour(chosenTour);
	...
	protected abstract CustomerWithCost calculateCostHook(Customer customer, Tour tour, GroupOfTours got, double approximateEquality);
